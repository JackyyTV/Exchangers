...
public class ExchangerHandler extends Item {
    ...
    public void addInformation(ItemStack stack, World world, List<String> tooltip, ITooltipFlag advanced) {
        ...
        ItemExchanger exchanger = (ItemExchanger) stack.getItem();
        ...
        if (compound == null || compound.getInteger("block") == 0) {
            ...
            tooltip.add(StringHelper.localize("tooltip.maxrange") + " " + modeSwitchList[exchanger.getTier().getMaxSize()]);
        }
        if (StringHelper.isShiftKeyDown()) {
    ...
    public void switchMode(EntityPlayer player, ItemStack stack) {
        ...
        if (heldItem != null) {
            if (modeSwitch > exchanger.getTier().getMaxSize())
                modeSwitch = MODE_1X1;
        }
    ...
    public EnumActionResult onItemUse(EntityPlayer player, World world, BlockPos pos, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
        if (!world.isRemote) {
            if (player.isSneaking()) {
                selectBlock(player.getHeldItem(hand), player, world, pos);
            } else {
                placeBlock(player.getHeldItem(hand), player, world, pos, side);
            }
        }
    ...
    private void placeBlock(ItemStack stack, EntityPlayer player, World world, BlockPos pos, EnumFacing side) {
        ...
        if (stack.getItem() instanceof ItemPoweredExchanger) {
        ...
        for (BlockPos coordinate : coordinates) {
            if (consumeItemInInventory(Item.getItemFromBlock(block), meta, player.inventory, player)) {
                if (!player.capabilities.isCreativeMode && !isCreative()) {
                    ...
                    if (stack.getItem() instanceof ItemPoweredExchanger) {
                        ItemPoweredExchanger powered = (ItemPoweredExchanger) stack.getItem();
                        if (stack.getTagCompound().getInteger("Energy") >= powered.getPerBlockCost()) {
                            stack.getTagCompound().setInteger("Energy", stack.getTagCompound().getInteger("Energy") - powered.getPerBlockCost());
                        }
                    }
                }
